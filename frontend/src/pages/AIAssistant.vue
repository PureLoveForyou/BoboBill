...
<template>
  <div class="h-[calc(100vh-2rem)] flex animate-elegant-in">
    <!-- 对话历史侧栏 -->
    <div class="w-72 flex-shrink-0 border-r border-elegant/50 flex flex-col glass-elegant rounded-2xl m-2 mr-0 shadow-elegant"
         :class="{ 'hidden': !showSidebar, 'lg:flex': showSidebar }">
      <div class="p-5 border-b border-elegant/50 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-gradient-elegant flex items-center justify-center shadow-lg">
            <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-3.582 8-8 8a8.818 8.818 0 01-6.95-4.06L3 13l3.05-.94A8 8 0 1112 21z"/>
            </svg>
          </div>
          <h2 class="font-bold text-sm tracking-tight">{{ t('ai.chatHistory') }}</h2>
        </div>
        <button @click="createChatSession()" class="btn btn-primary btn-sm gap-1 shadow-md hover-lift transition-elegant" :title="t('ai.newChat')">
      ...
      <div class="flex-1 overflow-y-auto p-3 space-y-1.5">
        <div
          ...
          class="group flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-elegant text-sm truncate hover-lift"
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <span class="truncate block font-semibold">{{ item.title }}</span>
            <span v-if="item.preview" class="truncate block text-[10px] opacity-50 mt-0.5">{{ item.preview }}</span>
          </div>
          <span class="text-[10px] flex-shrink-0 opacity-40">{{ formatTime(item.updated_at) }}</span>
          <button @click.stop="deleteChat(item.id)" class="opacity-0 group-hover:opacity-50 hover:!opacity-100 text-error transition-all p-1 rounded-lg hover:bg-error/10">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
            </svg>
          </button>
        </div>
        <div v-if="!chats.length" class="text-center text-xs text-base-content/30 py-12">
          <div class="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-base-200/60 to-base-100/40 flex items-center justify-center">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <div class="flex items-center justify-between p-5 pb-0">
        <div class="flex items-center gap-3">
          <button @click="showSidebar = !showSidebar" class="btn btn-ghost btn-sm lg:hidden p-2 hover:bg-base-200/50 rounded-xl transition-elegant">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16"/></svg>
          </button>
          <button @click="showSidebar = !showSidebar" class="btn btn-ghost btn-sm p-2 hidden lg:flex hover:bg-base-200/50 rounded-xl transition-elegant" :title="t('ai.toggleHistory')">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16"/></svg>
          </button>
          <div>
            <h1 class="text-2xl font-bold tracking-tight">
              <span class="bg-gradient-to-r from-primary via-purple-500 to-pink-500 bg-clip-text text-transparent">{{ t('ai.title') }}</span>
            </h1>
            <p class="text-xs text-base-content/40 mt-0.5" v-if="!isConfigured">
              <button @click="goToSettings" class="text-primary hover:underline font-semibold">{{ t('ai.notConfigured') }} {{ t('echo.goSettings') }}</button>
            </p>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <div v-if="isConfigured" class="relative">
            <select :value="activeConfigId" @change="selectConfig(parseInt($event.target.value))"
              class="select select-sm bg-base-200/50 border-elegant/50 pr-8 text-xs font-medium rounded-xl shadow-inset hover-lift transition-elegant">
              <option v-for="cfg in aiConfigs" :key="cfg.id" :value="cfg.id">
                {{ cfg.name }} ({{ cfg.model }})
              </option>
            </select>
          </div>
          <button @click="createChatSession()" class="btn btn-ghost btn-sm text-base-content/50 hover:text-primary gap-1 hover:bg-base-200/50 rounded-xl transition-elegant hover-lift" :title="t('ai.newChat')">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
            <span class="hidden sm:inline font-semibold">{{ t('ai.newChat') }}</span>
          </button>
          <button @click="clearCurrentChat" class="btn btn-ghost btn-sm text-base-content/50 hover:text-error gap-1 hover:bg-error/10 rounded-xl transition-elegant hover-lift" v-if="currentMessages.length" :title="t('ai.clearChat')">
            ...
            <span class="hidden sm:inline font-semibold">{{ t('ai.clearChat') }}</span>
          </button>
        </div>
      </div>

      <!-- Messages -->
      <div ref="chatContainer" @scroll="onChatScroll" class="flex-1 overflow-y-auto p-5 space-y-4 scroll-smooth relative">
        <div v-if="!currentMessages.length" class="flex flex-col items-center justify-center h-full text-center px-8">
          <div ref="evaContainerRef" class="eva-avatar mb-8 relative">
          ...
          <h2 class="text-2xl font-bold mb-3 tracking-tight">{{ t('ai.welcome') }}</h2>
          <p class="text-sm text-base-content/50 leading-relaxed max-w-md font-medium">{{ t('ai.welcomeHint') }}</p>
          <div v-if="!isConfigured" class="mt-8">
            <button @click="goToSettings" class="btn btn-primary btn-sm gap-2 shadow-elevated hover-lift transition-elegant group">
          ...
          <div v-else class="mt-8 grid gap-2 w-full max-w-md">
            <button v-for="suggestion in ['ai.suggest1', 'ai.suggest2', 'ai.suggest3']" :key="suggestion"
              ...
              class="text-left p-4 rounded-xl bg-base-200/50 hover:bg-base-200 hover-lift transition-elegant text-sm text-base-content/70 hover:text-base-content font-medium border border-elegant/30">
              {{ t(suggestion) }}
            </button>
          </div>
        </div>
        ...
        <template v-else>
          <div v-for="(msg, index) in currentMessages" :key="msg.id || index"
            class="flex gap-3 max-w-[85%] animate-elegant-in"
            :class="msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''"
            :style="{ animationDelay: `${index * 0.05}s` }">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 mt-1 shadow-lg"
              :class="msg.role === 'user' ? 'bg-gradient-elegant text-white' : 'eva-avatar-mini glass-elegant'">
              <span v-if="msg.role === 'user'" class="text-sm font-bold">你</span>
            ...
            <div class="rounded-2xl px-5 py-3.5 text-sm leading-relaxed break-words shadow-elegant"
              :class="msg.role === 'user' ? 'bg-gradient-elegant text-white rounded-tr-sm' : 'glass-elegant rounded-tl-sm'">
              <div v-if="msg.loading && !msg.content && !msg.reasoning && (!msg.toolCalls || !msg.toolCalls.length)" class="flex items-center gap-2 py-1.5">
              ...
              <div v-if="msg.toolCalls && msg.toolCalls.length" class="space-y-2 py-2">
                <div v-for="(tc, tcIdx) in msg.toolCalls" :key="tcIdx"
                  class="flex items-center gap-2.5 text-xs rounded-lg px-3.5 py-2 transition-all"
                  ...
                  <span class="font-semibold">{{ tc.description }}</span>
                </div>
              </div>
              <div v-if="msg.reasoning" class="mb-3">
                <button @click="msg.reasoningExpanded = !msg.reasoningExpanded"
                  class="flex items-center gap-2 text-xs text-primary/70 hover:text-primary transition-colors py-1 font-semibold hover-lift">
                  <svg class="w-4 h-4 transition-transform duration-200" :class="{ 'rotate-90': msg.reasoningExpanded }"
                  ...
                  <span class="font-bold">{{ t('ai.thinking') }}</span>
                ...
                <div v-show="msg.reasoningExpanded"
                  class="mt-2 pl-4 border-l-2 border-primary/20 text-xs text-base-content/50 leading-relaxed whitespace-pre-wrap break-words">{{ msg.reasoning }}</div>
              </div>
              <template v-if="msg.content">
                <div v-if="msg.content.startsWith('[错误]')" class="text-error font-semibold">{{ msg.content }}</div>
        ...
        <Transition name="fade">
          <button v-if="showScrollDownBtn" @click="scrollToBottom"
            class="absolute bottom-6 right-8 btn btn-circle btn-sm glass-elegant shadow-elevated hover-lift transition-elegant z-10">
            <svg class="w-5 h-5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 14l-7 7m0 0l-7-7m7 7V3"/></svg>
          </button>
        </Transition>
      </div>
      ...
      <!-- Input Area -->
      <div class="p-5 pt-0">
        <div class="glass-elegant rounded-2xl shadow-elegant-lg p-3 hover-lift transition-elegant">
          <div class="flex items-end gap-3">
            <textarea v-model="inputText" @keydown="handleKeydown" rows="1"
              ...
              class="flex-1 resize-none rounded-xl border-0 bg-transparent focus:bg-transparent focus:outline-none px-4 py-3 text-sm placeholder:text-base-content/30 transition-all disabled:opacity-50 font-medium shadow-inset"
              style="max-height:120px;min-height:52px;"
              @input="$event.target.style.height='auto'; $event.target.style.height=$event.target.scrollHeight+'px'" />
            <button v-if="isLoading" @click="stopGeneration"
              class="btn btn-circle btn-sm self-end flex-shrink-0 bg-error/10 text-error hover:bg-error hover:text-white shadow-md hover-lift transition-elegant" title="中断回答">
            ...
            <button v-else @click="handleSend" :disabled="!inputText.trim() || !isConfigured"
              class="btn btn-circle btn-sm self-end flex-shrink-0 bg-gradient-elegant text-white shadow-elevated hover-lift transition-elegant disabled:opacity-50 disabled:cursor-not-allowed group">
              <svg class="w-4 h-4 transition-transform group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5"/></svg>
            </button>
          </div>
          <p v-if="!isConfigured" class="text-xs text-warning mt-3 text-center font-medium">{{ t('ai.configHint') }} <button @click="goToSettings" class="text-primary hover:underline font-semibold">{{ t('echo.goSettings') }}</button></p>
        </div>
      </div>
    </div>
  </div>
