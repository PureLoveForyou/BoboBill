    ...
    >
      <div class="flex items-start justify-between mb-2">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center" :class="{
          'bg-[var(--bb-coral)]/10 text-[var(--bb-coral)]': stat.type === 'expense',
          'bg-[var(--bb-mint)]/10 text-[var(--bb-mint)]': stat.type === 'income',
          'bg-[var(--bb-blue)]/10 text-[var(--bb-blue)]': stat.type === 'balance',
          'bg-[var(--bb-bg-soft)] text-[var(--bb-text-secondary)]': stat.type === 'count'
        }">
        ...
        <div v-if="stat.trend !== 'neutral'" class="flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold" :class="{
          'bg-[var(--bb-mint)]/10 text-[var(--bb-mint)]': (stat.type === 'expense' ? stat.trend === 'down' : stat.trend === 'up'),
          'bg-[var(--bb-coral)]/10 text-[var(--bb-coral)]': (stat.type === 'expense' ? stat.trend === 'up' : stat.trend === 'down')
        }">
      ...
      <div class="text-xs font-medium text-[var(--bb-text-secondary)] mb-1">{{ stat.title }}</div>
      <div class="text-2xl font-bold tracking-tight text-[var(--bb-text)] mb-1">{{ stat.value }}</div>
      <div class="text-xs font-medium" :class="{
        'text-[var(--bb-coral)]': stat.type === 'expense',
        'text-[var(--bb-mint)]': stat.type === 'income',
        'text-[var(--bb-blue)]': stat.type === 'balance',
        'bb-muted': stat.type === 'count'
      }">{{ stat.desc }}</div>
    </div>
  </div>
</template>
